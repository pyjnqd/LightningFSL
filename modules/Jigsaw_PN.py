...
class Jigsaw_PN(BaseFewShotModule):
    ...
    def _batch_gather_ddp(self, images):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        images_gather = []
        for i in range(4):
            batch_size_this = images[i].shape[0] # 一个GPU上的batchsize 64
    ...
    def train_forward(self, batch, batch_size, way, shot):
        ...
        label_clu = torch.LongTensor(
            [j for j in range(way)] * (shot + self.hparams.num_query) * 4)
        ...
        return q_clu, label_clu, q_loc, label_loc, logits_PN
    ...
    def training_step(self, batch, batch_idx):
        q_clu, label_clu, q_loc, label_loc, logits_PN = self.train_forward(batch, self.hparams.train_batch_size_per_gpu,
                                                                           self.hparams.way, self.hparams.train_shot)
        """
        ==================Few Shot Learning=======================
        """
        label = torch.unsqueeze(self.label, 0).repeat(self.hparams.train_batch_size_per_gpu, 1).reshape(-1).to(
            logits_PN.device)
        ...
        loss = 0.5 * loss_PN + 0.25 * loss_clu + 0.25 * loss_loc
