...
def MetaOptNetHead_SVM_CS(query, support, n_way, n_shot, C_reg=0.1, double_precision=True, maxIter=3):
    ...
    support_labels = torch.eye(n_way).unsqueeze(0).repeat(n_shot, 1, 1).reshape(n_shot * n_way, -1).unsqueeze(0).repeat(
        support.size(0), 1, 1)
...
class MetaoptHead(nn.Module):
    ...
    def forward(self, query, support, n_way, n_shot, **kwargs):
        return self.head(query, support, n_way, n_shot, **kwargs)
