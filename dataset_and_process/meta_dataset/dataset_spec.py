...
from dataset_and_process import meta_dataset
from dataset_and_process.meta_dataset import imagenet_specification, learning_spec
import numpy as np
import six
...
def get_classes(split, classes_per_split):
  ...
  if split == "TRAIN":
    ...
    offset = classes_per_split["TRAIN"]
  elif split == "TEST":
    offset = (
        classes_per_split["TRAIN"] +
        classes_per_split["VALID"])
  else:
  ...
  def _get_split_offset(self, split):
    ...
    if split == "TRAIN":
      ...
      previous_superclasses = range(
          0, self.superclasses_per_split["TRAIN"])
      offset = self._count_classes_in_superclasses(previous_superclasses)
    elif split == "TEST":
      previous_superclasses = range(
          0, self.superclasses_per_split["TRAIN"] +
          self.superclasses_per_split["VALID"])
      offset = self._count_classes_in_superclasses(previous_superclasses)
    else:
      raise ValueError('Invalid dataset split.')
    return offset
...
def as_dataset_spec(dct):
  ...
  def _key_to_split(dct):
    ...
    return {
        key: value for key, value in six.iteritems(dct)
    }
  ...
  if dct['__class__'] == 'DatasetSpecification':
    ...
    for split_name, wn_id_counts in six.iteritems(dct['images_per_class']):
      ...
      images_per_class[split_name] = synset_counts
