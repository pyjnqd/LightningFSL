...
from dataset_and_process.meta_dataset import imagenet_stats
import numpy as np
import six
...
def create_splits(spanning_leaves, split_enum, valid_test_roots=None, **kwargs):
  ...
  #只包含所有叶子结点及祖先。且去除了.->.->.中的中间结点。每个graph的数据格式是包含synset的set
...
def get_num_synset_2012_images(args, path, synsets_2012, files_to_skip=None):
  ...
  This assumes that within args.ilsvrc_2012_data_root there is a directory for
  ...
  for s_2012 in synsets_2012:
    synset_dir = os.path.join(args.ilsvrc_2012_data_root, s_2012.wn_id)
...
def create_imagenet_specification(split_enum,
  ...
  data_root = args.ilsvrc_2012_data_root
  synsets = {}
  path_to_words = args.path_to_words
  ...
  path_to_is_a = args.path_to_is_a
  if not path_to_is_a:
  ...
  if train_split_only:
    ...
    #  split_enum.TEST: test_graph
    #      }
    #其中每个graph是对应split classes对应结点所有某结点之下的祖先结点
