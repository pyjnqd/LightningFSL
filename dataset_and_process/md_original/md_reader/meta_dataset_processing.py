...
class ImageDecoder(object):
  ...
  def __call__(self, example_string):
    ...
    image_decoded = tf.image.decode_image(image_string, channels=3, dtype=tf.uint8)
    ...
    # image_decoded = tf.cast(image_decoded, tf.int16)
    # image_decoded += 1
    # image_resized = tf.image.resize_image_with_pad(
    #     image_decoded, 84, 84, method=tf.image.ResizeMethod.BILINEAR, align_corners=False
    # )
    # image_resized -= 1
