...
from paths import META_RECORDS_ROOT, PROJECT_ROOT
...
TEST_METADATASET_NAMES = [ALL_METADATASET_NAMES[0]]
...
class MetaDatasetReader(object):
    def __init__(self, mode, train_set, validation_set, test_set):
        ...
        self.test_set_dict = {}
        self.specs_dict = {}
        gin.parse_config_file(f"{PROJECT_ROOT}/data/meta_dataset_config_v2.gin")
...
class MetaDatasetEpisodeReader(MetaDatasetReader):
    ...
    def _init_single_source_dataset(self, dataset_name, split, episode_description):
        ...
        single_source_pipeline = pipeline.make_one_source_episode_pipeline(
            ...
            image_size=84,
            shuffle_buffer_size=0)
        iterator = iter(single_source_pipeline)
        return iterator

    def _get_task(self, iterator):

        episode = next(iterator)[0]
        context_image_bytes = episode[0].numpy()
        target_image_bytes = episode[3].numpy()
        feature_description = {
        ...
        context_images = [
            Image.open(io.BytesIO(tf.io.parse_single_example(element, feature_description)['image'].numpy()))
            for element in context_image_bytes
        ]
        target_images = [
            Image.open(io.BytesIO(tf.io.parse_single_example(element, feature_description)['image'].numpy()))
            for element in target_image_bytes
        ]

        task_dict = {
            'context_images': context_images,
            'context_labels': torch.from_numpy(episode[1].numpy()).long().to(device),
            'context_gt': torch.from_numpy(episode[2].numpy()).long().to(device),
            'target_images': target_images,
            'target_labels': torch.from_numpy(episode[4].numpy()).long().to(device),
            'target_gt': torch.from_numpy(episode[5].numpy()).long().to(device)
            }
        return task_dict
        # task_dict = {
        #     'context_images': episode[0].numpy(),
        #     'context_labels': episode[1].numpy(),
        #     'context_gt': episode[2].numpy(),
        #     'target_images': episode[3].numpy(),
        #     'target_labels': episode[4].numpy(),
        #     'target_gt': episode[5].numpy()
        #     }
        # return self._to_torch(task_dict)
    def get_train_task(self):
        return self._get_task(self.train_dataset_next_task)

    def get_validation_task(self, item=None):
        item = item if item else list(self.validation_set_dict.keys())[0]
        return self._get_task(self.validation_set_dict[item])

    def get_test_task(self, item=None):
...
class MetaDatasetBatchReader(MetaDatasetReader):
    ...
    def _init_multi_source_dataset(self, items, split):
        ...
        multi_source_pipeline = pipeline.make_multisource_batch_pipeline(
            ...
            split=split, image_size=84, add_dataset_offset=True, shuffle_buffer_size=0)
